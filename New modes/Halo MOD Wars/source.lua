...
-- source for Halo MOD Wars v3, if a more up to date gamemode can be found on my fileshare you will have to ask me for a later version of the source code
...
alias min_dest_dropoff_dist = 360
alias rand_dest_dropoff_dist = 270
   ...
   if current_object.dropship_state == at_home then
      ...
      if temp_num0 == 0 or current_object == pickup_pelican then
         ...
         if current_object.standby_timer.is_zero() and dropship_initial_wait_timer.is_zero() then
            ...
            temp_obj0 = temp_obj2.place_at_me(bomb, none, none, 0,0,0,none)
            temp_obj4.delete()
            set_dropship_destination()
         end
      end
...
for each object with label "com_station" do
   if current_object.spawn_sequence == droppod_station then
      ...
      for each player do
         if current_object.team == current_player.team and current_player.state == none_ then
...
-- SPAWN ZONE STUFF
alias max_ai_per_team = script_option[1]
   ...
   if current_object.spawn_sequence == 0 then
      ...
      for each team do
         if current_team == current_object.team then
            if current_team.ai_count < max_ai_per_team then
