...
-- joint rotation stuff (Lower, tracking leg damage) 
...
alias R_has_scaled = object.number[5] -- priority local
...
on local: do 
   ...
   -- update the position of the scarab legs, both host & non-host
      ...
      --    temp_obj1.socket
   for each object with label "U_Joint" do
      if current_object.initialized == 1 and current_object.foot != no_object and current_object.LowerLength != 0 then
         ...
         --current_object.pitch.face_toward(current_object.foot, 0,0,0)
			...
			current_object.attach_to(current_object.socket, 0,0,0,relative)
			-- pop the pivot back so host doesn't mess with it
			curr_pitch.R_pivot.attach_to(curr_pitch, 0,0,0, relative)
