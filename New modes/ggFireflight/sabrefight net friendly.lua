...
declare global.number[5] with network priority low -- wave count
...
declare global.number[8] with network priority low -- bad guys counter
...
declare global.number[11] with network priority low -- difficulty set count
...
declare global.object[10] with network priority low -- game extra info object

declare global.object[11] with network priority local -- projectile mover
...
declare object.player[0] with network priority low -- vehicle target

declare object.number[0] with network priority local -- also spawner ai left to spawn -- also biped consecutive kills
declare object.number[1] with network priority local -- idk what this was used for
...
declare object.number[7] with network priority low -- i forgot which numbers i've used, phantom.is_in_use, also game.lives_left, also projectile tracking
...
declare player.number[7] with network priority low -- player.is_spectating

declare player.timer[0] = 3 -- MULTIKILL TIMER
declare player.object[3] with network priority low -- player attach to object
...
if global.object[10] == no_object then
   ...
   global.object[10].number[7] = script_option[4] -- starting lives
end
...
for each player do
   ...
   if temp_obj0 != no_object and current_player.number[2] == 1 then -- has biped & is initialized 
      if temp_obj0.object[0] == no_object and global.object[10].number[7] == 0 and current_player.number[6] == 1 then -- if not in a sabre & there are no spare lives & ahs already initially spawned
         ...
         if not current_player.biped.is_of_type(monitor) then
            ...
            temp_obj1.remove_weapon(primary, true)
      ...
      if current_player.number[7] == 1 then -- is no lives & spectating
         ...
         if current_player.timer[3].is_zero() then -- disable traits
            ...
            if temp_obj2.is_in_use() then
               ...
               current_player.object[3] = no_object
               global.player[0] = current_player
               for each player randomly do
                  temp_obj5 = current_player.biped
                  if current_player.number[7] == 0 and temp_obj5.object[0] != no_object and global.player[0].object[3] == no_object then -- is this a reasonable player to spectate?
                     global.player[0].object[3] = temp_obj5.object[0]
                  end
               end
            end
         end
      end
      if temp_obj0.object[0] == no_object and global.object[10].number[7] != 0 or current_player.number[6] == 0 then -- if there are lives left, or unlimited lives
         if global.object[10].number[7] > 0 and current_player.number[6] == 1 then
            global.object[10].number[7] -= 1 -- reduce number of active lives
            if global.object[10].number[7] == 0 then
               game.show_message_to(all_players, none, "No lives remaining")
            end
            if global.object[10].number[7] == 1 then
               game.show_message_to(all_players, none, "1 life remaining")
            end
            if global.object[10].number[7] == 2 then
               game.show_message_to(all_players, none, "2 lives remaining")
            end
            if global.object[10].number[7] == 3 then
               game.show_message_to(all_players, none, "3 lives remaining")
            end
            if global.object[10].number[7] == 5 then
         ...
         if current_player.biped.is_of_type(monitor) then -- set them to monitor mode to resolve a couple of issues
            temp_obj0 = current_player.biped -- its already set to this
      ...
      if temp_obj0.object[0] != no_object then 
         ...
         if temp_obj0.object[2] != no_object then 
            ...
            temp_obj6.set_shape(sphere, 285)
...
if global.object[10].number[7] == 0 and global.object[10].number[3] == 0 then -- no spare lives 
   temp_num0 = 0
   ...
   if temp_num0 > 1 then
      global.object[10].number[2] = 0 -- reset this value, so we can recall the last man standing incident 
   end
   if temp_num0 == 1 and global.object[10].number[2] == 0 then
      global.object[10].number[2] = 1 -- so we dont spam, we store this number
...
-- do a timer before ending the game
if global.object[10].number[3] > 0 then
   global.object[10].number[3] += 1
   if global.object[10].number[3] > 120 then -- 2 seconds
      game.end_round()
   end
end
...
do
   ...
   if host_existance_check.is_zero() then
      ...
      if local_player == no_player then
         host_indicator = 2
         script_widget[3].set_value_text("%n server pos updates\r\nDedicated Server ", client_corrections)
      end
   end
end
if local_player != no_player then
   script_widget[3].set_value_text("%n server pos updates\r\nLocally Hosted ", client_corrections)
end
...
function ai_update_pos_on_clients()
   ...
   -- move the hill_marker object forward (this interpolates the seraph/sabres position, as the host will correct this in a sec)
   current_object.object[2].attach_to(current_object, 3,0,0, relative)
   current_object.object[2].detach()
end

on local: do
   -- shrink spectators so they're not in the way
   for each player do
      if current_player.biped.is_of_type(monitor) then
         current_player.biped.set_scale(1)
         temp_obj1 = current_player.get_weapon(primary)
         temp_obj1.set_scale(1)
      end
   end
   -- prjectile mover script (this is so they never stop on clients)
   for each object with label "Active_Projectile" do
      -- scoot forward every tick
      if global.object[11] == no_object then
         global.object[11] = current_object.place_at_me(hill_marker, none, none, 0,0,0, none)
      end
      if current_object.number[7] == 0 then -- regular projectile speed
         if global.number[11] < script_option[12] then -- normal mode speed
            global.object[11].attach_to(current_object, 22,0,0,relative)
         end
         if global.number[11] >= script_option[12] then -- set +1 speed
            global.object[11].attach_to(current_object, 28,0,0,relative)
         end
      end
      if current_object.number[7] != 0 then -- tracking projectile speed
         if global.number[11] < script_option[12] then -- normal mode speed
            global.object[11].attach_to(current_object, 6,0,0,relative)
         end
         if global.number[11] >= script_option[12] then -- set +1 speed
            global.object[11].attach_to(current_object, 8,0,0,relative)
         end
      end
      global.object[11].detach()
      current_object.attach_to(global.object[11], 0,0,0,relative)
      current_object.detach()
   end
   -- spectator script 
   for each player do
      temp_obj0 = current_player.get_vehicle()
      if current_player.biped != no_object and temp_obj0 == no_object and current_player.number[7] == 1 then -- is no lives & spectating
         if current_player.object[3] != no_object then
            current_player.biped.attach_to(current_player.object[3], 5, 0, -10, relative)
   ...
   if local_player != no_player then 
      ...
      if host_indicator == 0 then
         ...
         -- local player health tracking script
         temp_obj0 = local_player.biped
         if temp_obj0.object[2] != no_object then
            temp_num2 = temp_obj0.object[2].health
            temp_num3 = temp_obj0.object[2].shields
            if global.object[10].number[0] > temp_num2 or global.object[10].number[1] > temp_num3 and global.object[10].number[0] > 0 then
               -- THE PLAYER HAS SINCE TAKEN DAMAGE (and isnt dead), PLAY DAMAGE EFFECTS
               temp_obj0 = local_player.biped.place_at_me(covenant_bomb, none, none, 0,0,0,none)
               temp_obj0.kill(false)
               temp_obj0.delete()
            end
            global.object[10].number[0] = temp_num2 -- health 
            global.object[10].number[1] = temp_num3 -- shields
         end
         -- rubberbanding fix script -- only vehicles
         if local_player.number[2] == 1 and script_option[15] == 1 then -- is not host and is registered && and is using rubberbanding fix
...
function ai_target_finding()
   temp_num0 = 32767 -- max distance
   current_object.object[1] = no_object
   --
   for each player do
      if current_player.biped != no_object and current_player.team != current_object.team and not current_player.biped.is_of_type(monitor) then
...
function ai_attempt_fire()
   ...
   if current_object.timer[1].is_zero() and temp_num1 < AI_max_firing_range and temp_num1 > 0 and current_object.timer[2].is_zero() and current_object.number[2] >= 0 then -- ENGAGEMENT RANGE
      ...
      if current_object.number[2] > 0 then
         ...
         if not current_object.is_of_type(sabre) and not current_object.has_forge_label("S_PelicanOBJ") then
            temp_obj0 = current_object.place_at_me(light_purple, "Active_Projectile", none, 15,0,0, none)
         end
         if current_object.is_of_type(sabre) or current_object.has_forge_label("S_PelicanOBJ") then
            temp_obj0 = current_object.place_at_me(light_red, "Active_Projectile", none, 15,0,0, none)
         end
         -- damage
      ...
      if current_object.number[2] == 0 then
         ...
         if not current_object.is_of_type(sabre) and not current_object.has_forge_label("S_PelicanOBJ") then
            temp_obj0 = current_object.place_at_me(light_green, "Active_Projectile", none, 15,0,0, none)
         end
         if current_object.is_of_type(sabre) or current_object.has_forge_label("S_PelicanOBJ") then
            temp_obj0 = current_object.place_at_me(light_white, "Active_Projectile", none, 15,0,0, none)
         end
         -- target tracking
         temp_obj0.object[3] = current_object.object[1] -- target to track
         temp_obj0.number[7] = 85 -- 85 ticks for tracking
         -- damage
         temp_obj0.number[2] = script_option[14] -- 40
         temp_obj0.set_scale(65)
      end
       -- offset projectiles forward from vehicles that would otherwise crash into them
      if current_object.has_forge_label("S_SeraphOBJ") or current_object.is_of_type(sabre) then
         temp_obj0.attach_to(current_object, 40,0,0,relative)
         temp_obj0.detach()
      end
...
-- object.object[0] is our client.fix_kungfu_obj -- only for sabres&seraphs, AKA PLASMA CANNON
...
function ai()
	if current_object.number[0] == 1 then
      ...
      if current_object.object[1] == no_object then
		...
		if current_object.object[1] != no_object then
         if temp_num0 == 32767 then -- did not find an inrange target
            temp_num1 = 1 -- so they are obliged to go towards them
         end
         if temp_num0 != 32767 then -- has found a valid range target
         ...
         if temp_num1 != 0 then -- then engage the target, also if they didn't find an inrange target because they were out of range
            basis = current_object
   ...
   if current_object.number[0] != 0 then
      ...
      if current_object.object[2] == no_object then
         current_object.object[2] = current_object.place_at_me(hill_marker, "S_cleanup", none, 0,0,0, none)
      ...
      if current_object.has_forge_label("S_PhantomOBJ") or current_object.has_forge_label("S_PelicanOBJ") then -- attempt firing mechanic 
         ...
         if current_object.timer[1].is_zero() then -- is ready to shoot
            ...
            if current_object.object[1] != no_object then
               ...
               ai_lookat()
               ai_attempt_fire()
               temp_obj0.copy_rotation_from(current_object.object[2], true) -- reorient projectile
            end
         end
      end
...
function AI_projectile()
   if temp_num2 == 0 and current_object.number[0] != 0 and current_object.team != temp_obj0.team then
      ...
      if temp_num0 <= target_size and temp_num0 > 0 then -- is in damage range and isn't on owners team
...
for each object with label "Active_Projectile" do
   ...
   if current_object.number[7] > 0 and current_object.object[3] != no_object then
      current_object.number[7] -= 1
      if current_object.number[7] == 0 then
         current_object.number[7] = -1 -- so we remember that this was a homing projectile
      end
   ...
   for each player do
      if temp_num2 == 0 then -- prevent execution if already hit someone, this is to prevent "ghost shots"
         ...
         if temp_obj0 != no_object and temp_obj0.object[2] != no_object then 
            temp_num0 = current_object.get_distance_to(temp_obj0)
            if temp_num0 <= 25 and temp_num0 > 0 then -- is in damage range
               -- apply damage
               ...
               temp_obj1.kill(true)
               if current_object.number[7] != 0 then -- is a heavy projectile
                  temp_obj1 = current_object.place_between_me_and(current_object, covenant_bomb, 0)
                  temp_obj1.kill(true)
               end
               current_object.number[0] = 9999
...
for each object with label 8 do
   current_object.number[0] += 1
   if current_object.number[0] >= 90 then
      current_object.delete()
   end
end

function biped_cleanup()
   -- dead body cleanup
   temp_num0 = current_object.health
   if temp_num0 <= 0 then
      current_object.delete()
   end
   -- if biped is still alive AND no body in the server claims responsibility for it, we delete it and its sabre and its other sabre stuff
   if temp_num0 > 0 then
      temp_num0 = 0 
      for each player do
         if current_player.biped == current_object then
            temp_num0 = 1
         end
      end
      if temp_num0 == 0 then
...
for each object with label 10 do -- elite bipeds, they should not exist though, so this is only fallback
   biped_cleanup()
end
for each player do
   temp_obj0 = current_player.biped
   if temp_obj0.is_of_type(monitor) then
      temp_obj0.number[5] = 999
   end
end
for each object with label 15 do
   if current_object.number[5] == 999 then
      temp_num0 = 0 
      for each player do
         if current_player.biped == current_object then
            temp_num0 = 1
            ...
            if temp_obj0 != no_object and current_player.number[2] == 1 and current_object.timer[3].is_zero() then 
               ...
               if temp_num1 == 3 then
                  ...
                  if temp_num1 == 0 then
                     for each object with label "S_PelicanOBJ" do
                        if current_object.number[0] == 0 and current_object.number[7] == 0 and temp_obj2 == no_object then 
                           current_object.number[7] = 1 -- in use
...
do
   ...
   if temp_num3 <= 0 and temp_num1 <= 0 then -- no enemies, new wave incoming
      if global.number[6] != global.number[5] then -- its a new wave
         ...
         for each player do
            if current_player.number[7] == 1 then --and current_player.number[6] == 1 then -- if they're a monitor, chuck a free initial spawn, so they can spawn without sucking up the lives we just awarded
         ...
         -- increment lives
         global.object[10].number[7] += script_option[6] -- bonus lives per live completion
         game.show_message_to(all_players, none, "Lives added")
         if global.object[10].number[7] > script_option[5] and script_option[5] > -1 then -- cap spare lives at maximum
            global.object[10].number[7] = script_option[5]
         end
      end
   end
   for each player randomly do
...
do
   temp_num0 = 0
   for each player do
      if current_player.number[2] != 0 then -- has been validated
         temp_num0 += 1
      end
   end
   for each player do
   ...
   if temp_num0 <= 2 then 
      script_widget[3].set_text("| 1-2 players balancing\r\n|\r\n|")
   end
   if temp_num0 > 2 and temp_num0 <= 5 then 
      script_widget[3].set_text("| 3-5 players balancing\r\n|\r\n|")
   end
   if temp_num0 > 5 and temp_num0 <= 10 then 
      script_widget[3].set_text("| 6-10 players balancing\r\n|\r\n|")
   end
   if temp_num0 > 10 then 
      script_widget[3].set_text("| 11-16 players balancing\r\n|\r\n|")
   end
end
...
do
   ...
   --script_widget[0].set_value_text("issues with: players going too far away,\r\nspectators dont properly respawn,\r\nseraphs dont count as kills\r\n\r\n\r\n\r\n ")
   script_widget[0].set_text("SPECTATING; NO LIVES LEFT")
   for each player do
   ...
   if script_option[4] > -1 then
      script_widget[1].set_value_text("%n Lives\r\n%n bad guys", global.object[10].number[7], global.number[8])
   end
   if script_option[4] == -1 then
