...
-- ########################
   ...
   -- frig attachments
      ...
      alias gravity_obj = object.object[2]
      alias placeholder_obj = object.object[1]
   ---
   ...
   -- frigate vars
      ...
      alias frig_gravity_obj = object.object[1]
      alias frig_mover_step = object.object[2]
...
-- ######################
   ...
   -- out of bounds variables
      ...
      declare player.out_of_bounds_timer = 30
   ---
   -- armory variables
...
----------------------------
   ...
   ----------------------------
      if ground_checker == no_object then
         ...
         ground_checker.offset = ground_checker.place_at_me(capture_plate, none, none, 0,0,0,none)
   ...
   -------------------
      for each object with label "frig_origin" do
         if current_object.frig_init == 0 then
            ...
            current_object.frig_direction_obj = current_object.place_at_me(hill_marker, none, none, 127,0,0, none)
            current_object.frig_mover_step = current_object.place_at_me(flag_stand, none, none, 0,0,0, none)
            current_object.frig_mover = current_object.place_at_me(monitor, none, none, 0,0,0, none)
   ...
   -----------------------------
      for each object with label "frig_origin" do
         if current_object.frig_gravity_obj == no_object then
            ...
            --temp_obj0.attach_to(current_object, 1,0,0,relative) -- for relative direction
            -- temp_obj0.detach() -- weird stuff happens when we remove htis an
            current_object.frig_mover.attach_to(current_object.frig_mover_step, 0,0,0,relative)
            ...
            for each object with label "frig_origin" do
               if temp_obj2 != current_object then -- and current_object.frig_gravity_obj == no_object then -- even push away from a frigate that is crashing!!!
                  ...
                  if temp_num0 < 600 then
                     ...
                     temp_obj2.frig_mover.attach_to(temp_obj0, -2,0,0,relative)
            ...
            temp_obj0.set_scale(1100)
            temp_obj0.copy_rotation_from(temp_obj0, true)
            current_object.frig_direction_obj.detach()
            -- cleanup helper object
            temp_obj0.delete()

            -- mark next position to move to
            current_object.frig_mover_step.attach_to(current_object, 1,0,0,relative)
            current_object.frig_mover_step.detach()
         end
      end
   ---
   ------------------------------------
   -- UPDATE DAMAGE OBJECT POSITIONS --
   ------------------------------------
      for each object with label "frig_damage" do
         if current_object.offset != no_object and current_object.placeholder_obj != no_object then
            current_object.placeholder_obj.copy_rotation_from(current_object.offset, true)
            current_object.placeholder_obj.attach_to(current_object.offset, 0,0,0,relative)
            current_object.placeholder_obj.detach()
         end
      end
   ---
   ----------------------------
      ...
      if temp_num3 == 0 then
         ...
         ---------------------
            for each player do
               if current_player.biped != no_object then
                  ...
                  if not current_player.last_frigate.shape_contains(current_player.biped) then
                  ...
                  -- make sure this player is on the same frigate as our local player, or else theres no point cascade attaching them


                  if temp_obj4 != no_object and current_player.last_frigate == local_player_.last_frigate or global_ticker > 0 then -- current_player == local_player_
                     temp_obj5 = current_player.get_vehicle()
                     if temp_obj5 == no_object or temp_obj5.has_forge_label("frig_attach") then
         ...
         --------------------------------
            if local_player_ != no_player then
               if global_ticker <= -300 then -- non-hosts
             --if global_ticker > 1000 then -- hosts
                  ...
                  if local_player_.last_frigate != no_object then
                     for each object with label "frig_attach" do
                        if current_object.parent_frig == local_player_.last_frigate and current_object.offset != no_object then
                           current_object.detach()
                        end
                     end
                  end
...
------------------------
   ...
   -----------------------
      ...
      if game.round_timer.is_zero() and game.round_time_limit > 0 and temp_num0 > 0 then 
...
--    temp_num0 += 1
-- end
-- script_widget[0].set_text("objects: %n, frig_dist %n", temp_num0, frigates_distance)
