-- random nubmer generation tests 2.0
...
function setup_data_obj() 
   if global.number[1] < data_objs_max then
      ...
      global.object[0].result = global.number[1]
      global.number[1] += 1
      setup_data_obj()
   end
end
...
if player[0].biped != no_object then -- we have someone to spawn our objects at
   ...
   global.number[1] = rand(2)
   for each object with label "data_obj" do 
      if global.number[1] == current_object.result then
         current_object.result_matched += 1
      end
   end

   -- update every X ticks
   --global.number[0] += 1
   --if global.number[0] >= 60 then
   --   global.number[0] = 0
