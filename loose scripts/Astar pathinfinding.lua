...
alias effiency = object.number[4]
      ...
      for each object with label "p_node" do
         if current_object != temp_obj0 then
            if temp_obj0.shape_contains(current_object) and current_object.shape_contains(temp_obj0) then -- they both overlap, hurray!
               ...
               if temp_num0 == 0 then 
                  game.show_message_to(all_players,  none, "Pathing ERROR: base exceeded 4 linked nodes, this will be bad for the economy")
               end
            end
         end
      end
...
function return_parent()
      ...
      if temp_obj0.best_parent >= 1 then 
         ...
         if temp_obj0.best_parent >= 2 then 
            ...
            if temp_obj0.best_parent == 3 then 
               ...
               -- technically this error is impossible to achieve, based on the code
               -- if temp_obj0.best_parent >= 3 then
               --    game.show_message_to(all_players,none, "Agent ERROR: some goober set an invalid best_parent value")
               -- end 
            end
         end
      end
   ...
   if temp_obj1 == no_object then
      game.show_message_to(all_players,none, "Agent ERROR: failed to resolve return_parent()")
   end 
end
-- inputs 
...
function write_parent()
   ...
   if temp_obj0.best_parent == -1 then
      game.show_message_to(all_players,none, "Agent ERROR: failed to resolve write_parent()")
   end 
end
...
function evaluate_node()
   if temp_obj3 == no_object then
      ...
      if temp_obj0 != no_object and temp_obj0.has_been_reached != 2 then 
         temp_obj0.team = team[0]
         temp_obj0.set_waypoint_visibility(everyone)
         temp_obj0.set_waypoint_icon(none) -- indicate that this node has now been reached
         ...
         if temp_obj0.best_parent != -1 then -- we have already evaluated this one, and must check to see if our current parent is more efficient
            ...
            if temp_num0 < temp_obj0.lowest_dist then
               ...
               -- this is for debugging, as we already calculate the efficieny when the time calls
               temp_obj0.effiency = temp_obj0.get_distance_to(temp_obj2)
               temp_obj0.effiency += temp_num0
               temp_obj0.effiency -= temp_num2
               --temp_obj0.effiency *= -1
         ...
         if temp_obj0.best_parent == -1 then -- no parent, and we're good to just slap our values in
            ...
            temp_obj0.effiency = temp_obj0.get_distance_to(temp_obj2)
            temp_obj0.effiency += temp_obj0.lowest_dist
            temp_obj0.effiency -= temp_num2
            --temp_obj0.effiency *= -1
...
function recurs_path_scan()
   ...
   if temp_obj1 == no_object then
      game.show_message_to(all_players,  none, "Agent ERROR: no valid priority#1 node")
   end
   if temp_obj1 != no_object then
      --  we now evaluate this node, and mark its child nodes for evalution when we do out next iteration of this code
      temp_obj1.team = team[1]
      temp_obj1.set_waypoint_visibility(everyone)
      temp_obj1.set_waypoint_icon(none)
...
function recurse_back_through()
   ...
   temp_obj0.set_waypoint_visibility(everyone)
   temp_obj0.set_waypoint_icon(diamond) -- mark that this node is a part of the correct path
   ...
   if temp_obj1 != temp_obj2 then
      ...
      if temp_obj1 == no_object then -- then we hit a dead end somehow?
         game.show_message_to(all_players,  none, "Agent ERROR: hit a dead end while backtracing")
      end
   end
   if temp_obj1 == temp_obj2 then 
...
for each object with label "agent" do
   ...
   if current_object.destination == no_object then -- no destination
   ...
   if current_object.destination != no_object then -- activate pathfinding protocol
      if current_object.next_node == no_object and current_object.repath_timer.is_zero() then
         ...
         if current_object.current_node == no_object then 
            game.show_message_to(all_players,  none, "Agent ERROR: no available current node")
         end
         if current_object.current_node != no_object then -- then our guy is on the navmesh 
            ...
            if current_object.destination_node == no_object then 
               game.show_message_to(all_players,  none, "Agent ERROR: no available destination node")
            end
            if current_object.destination_node != no_object then -- then we're both on the navmesh 
               ...
               if current_object.destination_node != current_object.current_node then
                  ...
                  for each object with label "p_node" do
                     ...
                     -- here we're displaying the actual distance from the current_node to the hud_target_node and the overall offset this path is from a striaght line, to the destination
                     current_object.set_waypoint_text("C:%n t:%n", hud_target_object.lowest_dist, hud_target_object.effiency)
                  ...
                  if temp_obj3 == no_object then 
                     game.show_message_to(all_players,  none, "Agent ERROR: no possible path")
                  end
                  if temp_obj3 != no_object then -- we successfully drew a line from start to finish
                     ...
                     recurse_back_through()
                     -- debug the role of these nodes in the path
                     current_object.destination_node.set_waypoint_visibility(everyone)
                     current_object.destination_node.set_waypoint_icon(arrow)
                     current_object.current_node.set_waypoint_visibility(everyone)
                     current_object.current_node.set_waypoint_icon(crown)

                     if temp_obj3 == no_object then
                        game.show_message_to(all_players,  none, "Agent ERROR: failed to backtrace path")
                     end
                     if temp_obj3 != no_object then
                        ...
                        temp_obj3.set_waypoint_visibility(everyone)
                        temp_obj3.set_waypoint_icon(padlock)
                     end
                  end
               end
            end
      ...
      if not current_object.repath_timer.is_zero() or current_object.next_node == no_object then -- blind mode
         ...
         current_object.set_waypoint_icon(bullseye)
         temp_obj0 = current_object.destination
         ai_movement_towards()
      ...
      if current_object.next_node != no_object and current_object.repath_timer.is_zero() then 
         ...
         current_object.set_waypoint_icon(inward) -- display that the AI is currently pathing
         ...
         if current_object.next_node.shape_contains(current_object) then -- signal find next node and go that way
            ...
            current_object.next_node = no_object
            game.show_message_to(all_players,  none, "DEBUG: reached next node")
         end
      end
   end
end